...
function Row({ channel, selected, onToggle }: { channel: SdxChannel; selected: boolean; onToggle(): void }) {
  ...
  return <tr ref={sortable.setNodeRef} style={{ transform: CSS.Transform.toString(sortable.transform), transition: sortable.transition }}>
    <td><button className="handle" {...sortable.attributes} {...sortable.listeners} aria-label="Sürükle">⠿</button></td><td><input type="checkbox" checked={selected} onChange={onToggle} /></td>
...
export default function App() {
  ...
  const [query, setQuery] = useState(''); const [kind, setKind] = useState('Tümü'); const [mode, setMode] = useState<BuildMode>('replace'); const [modal, setModal] = useState<SdxChannel[] | null>(null);
  ...
  return <main>
    ...
    {!file ? <section className="empty"><h2>Başlamak için bir veya daha fazla .sdx dosyası seçin</h2><p>İlk dosya düzenlenecek ana listedir; diğerleri aranabilir yerel katalogya eklenir.</p><button className="primary" onClick={() => input.current?.click()}>Dosya seç</button></section> : <>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={({ active, over }) => { if (!over || active.id === over.id) return; const a = channels.findIndex(c => c.id === active.id), b = channels.findIndex(c => c.id === over.id); setChannels(arrayMove(channels, a, b)); }}><table><thead><tr><th></th><th></th><th>Kanal</th><th>Tür</th><th>Frekans</th><th>Pol.</th><th>SR</th><th>Service ID</th><th>Sürüm</th></tr></thead><tbody><SortableContext items={visible.map(c => c.id)} strategy={verticalListSortingStrategy}>{visible.map(channel => <Row key={channel.id} channel={channel} selected={selected.has(channel.id)} onToggle={() => toggle(channel.id)} />)}</SortableContext></tbody></table></DndContext>
